...
- When real pet content lands, its shape/contract is
  ...
  instead (see §13).
- **Product UI chrome is first-party procedural geometry only** (Block
  12A, DEC-142..DEC-146). No image assets (backgrounds, decorative
  PNGs, icon packs, SVG packs), **no bundled fonts**, no gradient/
  texture dependencies in `src/productui`: ornaments, panels, the
  wallet pill and the wordmark are drawn from geometry via
  `productui::Ornaments` over the platform-safe native text path.
  Colors come from the semantic tokens (`productui::tokens`), per-pet
  identity from the one central registry (`productui::PetAccent`), and
  typography from tokenized font roles (`productui::type::FontRole`) —
  the editorial "serif" role resolves to the **system serif design**
  (New York on macOS, via `NSFontDescriptor`; falls back to sans),
  never a distributed font file. Scenic hero backgrounds (Block 12C)
  will be the first sanctioned Product UI image, with a procedural
  fallback — until then, none. See `docs/PRODUCT_UI.md` §23.
