...
| `docs/PRODUCT_UI.md` | Product shell: the Collection window (hero + gallery composition, per-pet accent hero stage, two visual planes, soft accented action button), lightweight compiled previews (`.nvprev`), Retina text path, ownership model, native quick menu, size/opacity/lock/hide controls, EN/ES localization + editorial contract, Pet Runtime vs Product UI vs System Shell, lifecycle, performance model, platform scope (Block 06 / 06.1 / 06.2). |
